...
public class Form5Controller {
    ...
    {
    	//Member 객체로 받는다(리퀘스트 파라미터 값이 너무 많을 때 새로운 객체 생성해서 받는게 유용함)
        String errorMsg = null;
        if (member.getUserid() == null || member.getUserid().length() == 0)
            ...
            session.setAttribute("member", member);//sessionAttribute 전달할때도 객체(member)로 전달
            return "redirect:register_success2";
        }
        model.addAttribute("member", member);//modelAttribute 전달할때도 객체(member)로 전달
        model.addAttribute("errorMsg", errorMsg);
